...
Use one line of code to evaluate the function values for all iterations ```xs``` Plot these values.

**Hint:** you need to iterate via ```eachcol(xs)``` or ```eachrow(xs)``` depending on how you represent ```xs```.
