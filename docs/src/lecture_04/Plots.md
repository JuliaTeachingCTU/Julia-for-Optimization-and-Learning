...
The [Plots](https://github.com/JuliaPlots/Plots.jl) package is not a standard plotting package as known from other languages. The Plots package provides a unified interface and toolset for creating plots, and the plots themselves are created by different [backends](http://docs.juliaplots.org/latest/backends/), like GR, PyPlot, PGFPlotsX, or Plotly. If one backend does not support desired features it is possible to easily switch to another backend with one command. No need to change the code. No need to learn a new syntax.

!!! warning "Time to the first plot"
    Compare to languages like Python or Matlab, it takes a lot of time to create the first plot in a fresh Julia session. The reason is, that Julia is compiled language. In Julia, all functions are compiled at their first run and the result is, that the first run of a function is slow.

The core of the Plots package is the `plot` function, which provides an interface for creating all types of plots. The most basic plot style is a line style which is the basic plot style provided by the `plot` function. The line plot can be created by calling the plot function on two vectors of numbers
...
The plot is displayed in a plot pane, a stand-alone window or the browser, depending on the environment and backend, see oficial [documentation](http://docs.juliaplots.org/latest/tutorial/#plotting-backends) for more details. Each column in the given arguments to the plot function is treated as a separate plot series, i.e., a set of related points that form line, surfaces, or any other plot type. Thus it is possible to create multiple plots at once
...
However, in many cases, it is more convenient to add new plots to the existing figure. This can be done using the `plot!` function which modifies the existing figure

```@example plots
plot!(x, sin.(x .+ π/4))
```

By default, the Plots package determines which plot is current using uses the global variable `Plots.CURRENT_PLOT`. However, it is possible to pass figure that should be modified as follows
...
So far, we created multiple simple plots without any change of style. In reality, we typically want to change the plot appearance based on our preferences. The Plots package provides a large number of plot attributes that change how the final plot looks like. The package follows a simple rule with data vs attributes: positional arguments are input data, and keyword arguments are attributes.
...
Note that it is possible to use column vectors and row vectors as attributes at the same time. In the following example, we add a cosine function into the previous plot and set the color for the cosine function to be red.

```@example plots
plot(x, [sin.(x) cos.(x)]; linewidth, color = [color :red], label, xlims, ylims)
```

There are a large number of attributes. The Plots package provides the `plotattr` to print a list of all attributes for either series, plots, subplots, or axes

```@repl plots
plotattr(:Series)
```

The `plotattr` function accepts any of the following arguments: `:Plots`, `:Series`, `:Subplot` and `:Axis`. The `plotattr` function can also be used to print a description of a concrete attribute.

```@repl plots
plotattr("title")
```

Note that in this case, we use a `String` instead of `Symbol`. Be aware, that not all attributes are supported. For example, attributes that can be specified for different axis such as `xguide`, `yguide` are not usually supported
...
**Hint:** use the `pallete` function  in combination with the `collect` function to generate a vector of colors from `:viridis` color scheme.
...
However, as can be seen in the example above, the `palette` function returns the `ColorPalette` type. It is perfectly fine in most cases. However, in our case, we want to concatenate two vectors of colors together. So we have to use the `collect` function to extract the vector of colors from the `ColorPalette` type
...
Finally, we can call the `plot` function with input arguments and attributes defined above. Note tha we use `axis = nothing` and `border = :none` to remove all decorators such as ticks or axis frame
...
In the previous section, we used the sine and cosine functions to show the basic functionality of the `plot` function. In all previous examples, we first calculated the values of the functions and then created the graphs. However, it is possible to pass functions directly to the `plot` function
...
Instead of the vector of values, it is also possible to use similar syntax as for ranges and to pass the starting, stopping point and length (the length is optional)
...
This exercise is very simple. The first thing that we have to do is to define functions described by the set of equations in the exercise description
...
In the previous sections, we used only line plots. However, there are many other series types such as scatter plots, heatmaps or contours. One way how to change the plot series is to use the `seriestype` attribute. In the following example, we create a plot of a sine function using the `scatter` series type
...
Draw this function for ``x, y \in [-5, 5]``. Use the following three plot series `contourf`, `heatmap` and `surface` with following settings:
- `:viridis` color scheme,
- camera angle `(25, 65)`,
- no legend,
- no colorbar,
- no decorators, i.e., no `axis`, `frame` or `ticks`.
...
Now, since we want to create three different plots with the same attributes, we can create a named tuple with the attributes. It will allow us to reuse the attributes
...
In many cases, it is useful to create a plot with a specific layout that contains multiple subplots. The Plots package provides the `layout` keyword to do it. The basic usage is as follows
...
All examples above can be also created incrementally. For example, we can create three separate plots as follows
...
Note that we use the `line_z` keyword that allows applying different colors to the different points of the curve in an easy way. When we are happy with the appearance of each plot, we can use the `plot` function and the `layout` keyword to create a final plot
...
The last thing we will discuss in this lecture is creating animations. In the following example, we will show how to update existing curve. The first thing we have to do is to create an empty plot a set all attributes we want
...
Finally, we can use simple `for` loop and the `frame` function to create an animation. On the second line, we use the `push!` function to append new points to the `plt` plot we defined before. The `frame` function captures the actual state of the `plt` plot and creates a new frame to the animation
...
When the animation is created, we can save it as a gif using the `gif` function as follows
...
In the following example, we will create an animation of the surface defined above where we will change the position of the camera in each frame. To change the camera position, we can use the `plot!` function and the `camera` keyword arguments as follows
